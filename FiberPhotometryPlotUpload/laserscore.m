function [base_stim,pre_stim,post_stim,lick_matrix]=laserscore(laserlevel,SampleFrequency,TIMEarg,BOUT_TIME_THRESHOLD,MIN_stimulation_THRESH)
...
if nargin<5;
    MIN_stimulation_THRESH=2;% % four licks or more make a bout
end
...
SampleTotaltime=length(laserlevel)/SampleFrequency;
xtime=0:SampleInterval:SampleTotaltime-SampleInterval;
%% lick time
lickleveldiff=diff(laserlevel);
...
subplot(3,1,1)
plot(xtime,laserlevel,'color',cyan);
xlim([0,xtime(end)]);
...
lick_boutson((licks_array < MIN_stimulation_THRESH)) = [];
lick_boutsoff((licks_array < MIN_stimulation_THRESH)) = [];
lick_boutsdata((licks_array < MIN_stimulation_THRESH)) = [];
...
for i = 1:trials
    ...
    if lick_boutson(i) < (PRE_TIME+BASE_TIME)
        ...
        lick_snips{i} = laserlevel(base_stim(i):post_stim(i))';
        Base_snips{i}=laserlevel(base_stim(i):pre_stim(i))';               
    end
end
